...
import { getStoredLeagueId, getStoredPlayerId } from '@/lib/storage-client';
...
export default function TablePage() {
  ...
  return (
    <div className="flex min-h-full flex-col">
      <SiteHeader leagueReady playerLoggedIn={playerLoggedIn} />
      <main className="mx-auto w-full max-w-4xl flex-1 px-4 py-8">
        {data?.welcome_message && (
          <p className="mb-4 rounded-lg bg-emerald-50 px-4 py-3 text-emerald-900 dark:bg-emerald-950/50 dark:text-emerald-100">
            {data.welcome_message}
          </p>
        )}
        <h1 className="mb-2 text-2xl font-bold">League table</h1>
        {data?.info_message && (
          <p className="mb-6 text-sm text-zinc-600 dark:text-zinc-400">{data.info_message}</p>
        )}
